...
export function Textarea({ invalid, className, rows = 4, ...rest }: Props) {
  return (
    <textarea
      ...
      className={clsx(
        /* 16px below sm keeps iOS Safari from zooming the page on focus. */
        "block w-full min-w-0 resize-y rounded-lg border bg-sheet px-3 py-2 text-[16px] leading-relaxed text-ink placeholder:text-ink-3 outline-none transition-colors sm:text-sm",
