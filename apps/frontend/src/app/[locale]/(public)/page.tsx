...
import { type Checklist, JURISDICTIONS, LANGUAGES, pick } from "@sahayak/shared";
import { GLOSSARY } from "@/lib/mock-data";
import { backendPublic } from "@/lib/backend";
...
export default async function LandingPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  // Live numbers from the backend; fallbacks keep the page up when it is down.
  const [procedures, live] = await Promise.all([
    backendPublic<{ procedures: Checklist[] }>("/api/procedures"),
    backendPublic<{ sections: number }>("/api/stats"),
  ]);
  const CHECKLISTS = procedures?.procedures ?? [];
  const indexed = live?.sections ?? 1000;
  const stats = [
